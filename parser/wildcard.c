...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/22 22:09:29 by lizard            #+#    #+#             */
/*   Updated: 2022/03/28 22:56:24 by lizard           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*find_in_ls(char *start, char *end)
{
	...
	while (i[1] > 0)
	{
		...
		if ((ft_strncmp(tmp[0], start, ft_strlen(start)) == 0
				|| ft_strlen(start) == 0) && (ft_strlen(end) == 0
				|| ft_strlen(tmp[0]) >= ft_strlen(start) + ft_strlen(end))
			&& (ft_strncmp(&tmp[0][ft_strlen(tmp[0]) - ft_strlen(end)],
				end, ft_strlen(end)) == 0))
		{
			tmp[2] = tmp[1];
